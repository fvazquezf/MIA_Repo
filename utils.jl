#=
This file consolidates utility functions from the MIA_Repo Jupyter notebooks.
It includes functions for:
- Data Preprocessing (one-hot encoding, normalization)
- Model Training (building and training ANNs)
...
function trainClassANN(
    ...
    loss(model,x,y) = (size(y, 2) == 1) ? logitbinarycrossentropy(model(x), y) : crossentropy(model(x), y)
    ...
    epochs_no_improve = 0
    best_ann_params = deepcopy(Flux.params(ann))
    ...
    if !isempty(validationInputs)
        Flux.loadparams!(ann, best_ann_params)
    end
